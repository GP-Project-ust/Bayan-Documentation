...
├── chapters/
│   ├── chapter1.tex              ← الفصل الأول: المقدمة (مكتمل — يتضمن مخططَي جانت)
│   ├── chapter2.tex              ← الفصل الثاني: الخلفية النظرية ومراجعة الأدبيات (مكتمل)
│   ├── chapter3.tex              ← الفصل الثالث: المنهجية وتحليل النظام (مكتمل)
│   │   └── chapter3/             ← أقسام الفصل الثالث الستة (منهجية، جدوى،
│   │                                 متطلبات، حالات استخدام، مخاطر، ملخص)
│   ├── chapter4.tex              ← الفصل الرابع: تصميم النظام (هيكل — مشروع التخرج 2)
│   ├── chapter5.tex              ← الفصل الخامس: تنفيذ واختبار النظام (هيكل — مشروع التخرج 2)
│   ├── chapter6.tex              ← الفصل السادس: النتائج والمناقشة (هيكل — مشروع التخرج 2)
│   └── chapter7.tex              ← الفصل السابع: الخاتمة والأعمال المستقبلية (هيكل — مشروع التخرج 2)
├── frontmatter/
│   ├── cover.tex                 ← صفحة الغلاف (الأسماء، الشعار، العنوان)
│   ├── declaration.tex           ← إقرار المشرف (يتضمن جدول الطلاب والأرقام الجامعية)
│   ├── examination_committee.tex ← لجنة المناقشة
...
│   └── tikz/                     ← 12 مخططاً بصرياً بصيغة TikZ (تُحوَّل عند البناء):
│       ├── agile-cycle.tex           ← دورة أجايل
│       ├── project-sprints.tex       ← خارطة الدورات التطويرية
│       ├── rag-flow.tex              ← تدفق RAG
│       ├── multi-agent-arch.tex      ← معمارية الوكلاء المتعددين
│       ├── usecase-actors.tex        ← تصنيف الفاعلين
│       ├── usecase-diagram.tex       ← مخطط حالات الاستخدام
│       ├── requirements-*.tex        ← مخططات المتطلبات (أولوية/جودة/نشر)
│       └── risk-*.tex                ← مخططات المخاطر (مصفوفة/توزيعات)
...
ج: عدّل ملفين: `cover.tex` (الغلاف) و`declaration.tex` (جدول الطلاب والأرقام الجامعية في إقرار المشرف)، ثم حدّث `pdfauthor` في `main.tex` إن تغيّر المعد الأول.
