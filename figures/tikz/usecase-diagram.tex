...
% Layout: human actors (KIND 1 — external) on the RIGHT, five AI
% agent actors (KIND 2 — internal: Supervisor + SQL + RAG +
% Analyst + Reporter) on the LEFT, 8 use cases in a single central column
% split into two tinted zones (user services / administration).
% The internal "Route Query" hub connects to the supervisor agent
% (execution line), which delegates to the four specialist agents
...
\begin{tikzpicture}[
  ...
  % ---- AI AGENT ACTORS (left side — 5 types) ----
  ...
  \pic at (1.45,-4.05) {robot={kausecondary!90!black}};
  \node[agentchip, below=3pt] at (1.45,-5.02)
    {\textarabic{وكيل قواعد البيانات}\\[1pt]{\tiny\EN{SQL Agent}}};

  \pic at (1.45,-6.35) {robot={kausecondary!90!black}};
  \node[agentchip, below=3pt] at (1.45,-7.32)
    {\textarabic{وكيل الاسترجاع}\\[1pt]{\tiny\EN{RAG Agent}}};

  \pic at (1.45,-8.65) {robot={kausecondary!90!black}};
  \node[agentchip, below=3pt] at (1.45,-9.62)
    {\textarabic{وكيل التحليل}\\[1pt]{\tiny\EN{Analyst Agent}}};

  \pic at (1.45,-10.95) {robot={kausecondary!90!black}};
  \node[agentchip, below=3pt] at (1.45,-11.92)
    {\textarabic{وكيل التقارير}\\[1pt]{\tiny\EN{Reporter Agent}}};
  ...
  \draw[deleg] (2.90,-1.75) -- (2.90,-10.95);
  \draw[deleg] (2.90,-1.75) -- (1.80,-1.75);
  \draw[deleg, -{stealth[scale=0.75]}] (2.90,-4.05) -- (1.85,-4.05);
  \draw[deleg, -{stealth[scale=0.75]}] (2.90,-6.35) -- (1.85,-6.35);
  \draw[deleg, -{stealth[scale=0.75]}] (2.90,-8.65) -- (1.85,-8.65);
  \draw[deleg, -{stealth[scale=0.75]}] (2.90,-10.95) -- (1.85,-10.95);

  % ---- Legend ----
  \node[draw=dividerdim!80, fill=white, rounded corners=3pt, inner sep=6pt,
        anchor=north west, font=\tiny, text=kaudark] at (0.10,-12.55)
  {%
    \begin{tabular}{@{}c@{\hspace{5pt}}r@{}}
      \tikz{\pic[scale=0.62, yshift=0.02] {stickman={kaudark}};} & \textarabic{فاعل بشري خارجي} \\[3pt]
      \tikz{\pic[scale=0.62] {robot={kausecondary!90!black}};} & \textarabic{فاعل داخلي: وكيل ذكاء اصطناعي} \\[3pt]
