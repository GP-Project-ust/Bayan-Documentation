...
% Engine: LuaLaTeX (required for clean Arabic text extraction)
%==============================================================================
...
% NOTE: previously \arabicfontfancy pointed to "Noto Naskh Arabic", which is
% not available in the xu-cheng/latex-action container. It is never used
% elsewhere in the document, so we point it at Amiri to avoid the
% fontspec lookup error at preamble-load time.
    ...
    \begin{tikzpicture}[remember picture, overlay]
      ...
      \node[anchor=center] at ($(current page.center) + (0, 1.5)$) {
        \begin{minipage}{0.85\textwidth}
          ...
          % Big Arabic Ordinal (الرابع, etc)
          {\color{kauprimary}\fontsize{65pt}{72pt}\selectfont\bfseries\kauDivOrdinal\par}
          \vspace{26pt}
          ...
          {\color{kaudark}\LARGE\bfseries\kauDivArTitle\par}
          \IfNoValueTF{#2}{}{%
            \vspace{10pt}
...
%   The previous XeLaTeX-based build produced garbled text on copy because
%   Amiri's ToUnicode CMap maps shaped Arabic glyphs to Arabic Presentation
%   Forms (U+FE70-U+FEFF) instead of base Arabic codepoints (U+0600-U+06FF),
%   and the text was stored in visual order. Switching to LuaLaTeX fixes
%   this at the source: LuaTeX generates ToUnicode CMaps that point directly
%   to the original base Arabic codepoints in logical order, so copied text
%   is clean, searchable, and correctly ordered in all PDF viewers.
%   The `unicode=true` / `pdfencoding=auto` hyperref options above remain
%   in place because they ensure PDF bookmarks (outline) and the Info
%   dictionary are Unicode-encoded.
%------------------------------------------------------------------------------

%------------------------------------------------------------------------------
% 5) Languages & bidirectional text — MUST be loaded before biblatex
%------------------------------------------------------------------------------
% Use polyglossia's built-in bidi=basic (LuaTeX native node-level bidi)
% instead of the standalone `bidi` package (which is XeTeX-only via
% \RequireXeTeX). polyglossia still provides \setRTL / \setLTR under this mode.
\usepackage[bidi=basic]{polyglossia}
...
%------------------------------------------------------------------------------
% Front matter — order per Ministry of Higher Education guide (2022):
%   1. Cover
%   2. Abstract (Summary) — Arabic + English
%   3. Authorization (English)
%   4. Dedication
%   5. Acknowledgment
%   6. Student Declaration + Supervisor Certification
...
\input{frontmatter/authorization}
\input{frontmatter/dedication}
\input{frontmatter/acknowledgment}
\input{frontmatter/declaration}
\input{frontmatter/examination_committee}
